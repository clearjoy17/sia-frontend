<template>
  <div>
    <b-button v-b-modal.stockEntryModal variant="primary">Add Stock</b-button>
    <b-modal id="stockEntryModal" title="Stock Entry" ok-title="Save Stock" @ok="onSubmit">
      <form action="#">
        <b-form-group
          label="Item Name"
          label-for="item_name"
...
export default {
  ...
  methods: {
    async onSubmit() {
      this.$axios.post('/api/stocks', this.stock)
      .then((res)=>{
        if(res.status==202) {
